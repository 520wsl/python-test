...
class load_book(object):
    def __init__(self, host, pageSize, loops, maxErrorSize):
        self.b_host = host
    ...
    def item_get_and_save(self, link, group, num):
        content = self.parseWithXpath(self.getHtmlText(self.b_link_load(link)))
        getBooksInfoData = self.getBooksInfo(content)
        # self.content.append(getBooksInfoData)
        print("\t\t第 %d 组链接， 第 %d 条链接 [ %s ] 抓取完成...\n" % (group, num, link))
    ...
    def contents_load(self):
        ...
        print("\t预计总共抓取小说 %s  本。\n" % (self.b_pageCount * self.b_pageSize))
        print('\t共分为 %s 个线程组 , 每组线程 %s 条链接。 \n' % (self.b_loops, task_size))
...
if __name__ == '__main__':
    book = load_book('http://www.xs8.cn', 500, 100, 2)
    book.contents_load()
    book.isOK()
