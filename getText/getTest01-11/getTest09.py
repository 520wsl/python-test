...
# -*- coding: utf-8 -*-
"""
__title__ = '爬取 网站 https://www.xs8.cn 小说 书籍信息 【书名、book_Id】 '
...
class load_book(object):
    ...
    def b_pageCount_load(self):
        try:
            ...
            print(html.xpath('//*[@id="page-container"]/@data-total'))
            data_total = html.xpath('//*[@id="page-container"]/@data-total')[0]
            count = math.ceil(float(data_total) / self.b_pageSize)
            return count
        except:
            print('%s :[error] 可能被拦截了 60 秒后再试试 !' % moment.now().format('YYYY-MM-DD HH:mm:ss'))
            time.sleep(60)
            self.b_pageCount
    ...
    def getHtmlText(self, url):
        ...
        heads[
            'User-Agent'] = 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/21.0.1180.89 Safari/537.1 QIHU 360SE'
        ...
        try:
            ...
            print("%s : [error] 获取列表失败,一秒后重试，失败链接：%s" % moment.now().format('YYYY-MM-DD HH:mm:ss'), url)
            time.sleep(1)
    ...
    def item_get_and_save(self, link, group, num):
        ...
        print("\t\t%s : [info] 第 %d 组链接， 第 %d 条链接 [ %s ] 抓取完成...【  %s  】\n" % (
            moment.now().format('YYYY-MM-DD HH:mm:ss'), group, num, link, len(getBooksInfoData)))
        if len(getBooksInfoData) < self.b_pageSize and len(getBooksInfoData) <= 2:
            self.errorurls.append(link)
            second = random.randint(60, 180)
            time.sleep(second)
            print('\n\t\t\t\t %s : [error] 网站可能被拦截， 自动 等待 %s 秒后 继续，采集数据。' % (
                moment.now().format('YYYY-MM-DD HH:mm:ss'), second))
    ...
    def section_load(self, links, group):
        if len(links) <= 0:
            return;
        print('\t %s : [info] 第 %d 组链接， 开始 抓取。。。\n' % (moment.now().format('YYYY-MM-DD HH:mm:ss'), group))
    ...
    def contents_load(self):
        ...
        print("时间 : %s" % moment.now().format('YYYY-MM-DD HH:mm:ss'))
        ...
        for i in nloops:
            if len(task_list[i]) <= 0:
                continue
            print('\n %s : [info] 开启第 %s 线程...' % (moment.now().format('YYYY-MM-DD HH:mm:ss'), i + 1))
            threads[i].start()
    ...
    def write_txt(self, getBooksInfoData, link, group, num):
        print("\t\t %s : [info] 第 %d 组链接， 第 %d 条链接 [ %s ] : ==> 开始 存储 链接内容...\n" % (
            moment.now().format('YYYY-MM-DD HH:mm:ss'), group, num, link))
        getBooksInfoRes = self.saveBooks(getBooksInfoData)
        if not getBooksInfoRes:
            print('\t %s : [info] 书籍信息存储失败')
        else:
            self.okNum += 1
            print("\t\t %s : [info] 第 %d 组链接， 第 %d 条链接 [ %s ] : ==> \t存储成功 %d 条 小说！\n" % (
                moment.now().format('YYYY-MM-DD HH:mm:ss'), group, num, link, len(getBooksInfoData)))

    def setLinks(self):
        self.errorSize += 1
        self.b_links = self.errorurls.copy()
        print('\t\t %s : [info] 存在失败链接 %s 条， 现在 【第 %s 次  】  重新抓取失败链接' % (
            moment.now().format('YYYY-MM-DD HH:mm:ss'), len(self.errorurls), self.errorSize))
...
if __name__ == '__main__':
    book = load_book('http://www.xs8.cn', 1000, 1, 10, 20)
    '''
        load_book
            网址
            分页大小
    ...
    book.contents_load()
    book.isOK()
    print(' %s : [info] 成功抓取链接 %s 条' % (moment.now().format('YYYY-MM-DD HH:mm:ss'), book.okNum))
    print(' %s : [info] 失败链接 %s 条：\n' % (moment.now().format('YYYY-MM-DD HH:mm:ss'), len(book.errorurls)))
