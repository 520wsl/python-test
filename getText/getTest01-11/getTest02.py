...
def directory(url):
    ...
    heads['Referer'] = 'https: // www.xs8.cn / chapter / 12284740304591203 / 1546996387119'
    heads[
...
# 数据库信息
def openMySqlConfig():
    return pymysql.connect("172.30.34.210", "root", "root", "pythonTest")


# 批量添加 信息
def batchAdd(sql, data_info):
    # sql
    # return openMySql(sql, data_info)
    db = openMySqlConfig()
    # 使用cursor()方法获取操作游标
    cursor = db.cursor()
    try:
...
def saveBooks(data_info):
    ...
    return batchAdd(sql, data_info)


def saveLinks(data_info):
    # data_info = [('32976698553944625', '12284740304591203', '第四章 偏偏男人像是逗弄她上瘾似的', '2', '3', '0'),
    #              ('32976681908229821', '12284740304591203', '第四章 偏偏男人像是逗弄她上瘾似的', '1', '3', '0')]
...
# bookData = directory("https://www.xs8.cn/ajax/chapter/userChapterList?_csrfToken=&bookId=12284740304591203&_=1546996387119")

# saveBooks(bookData)
# saveLinks()
# sql = '''INSERT INTO `books` (`bookId`, `bookName`, `chapterTotalCnt`) VALUES ('12284740304591203', '以婚谋爱', '67')'''
...
# 获取书本数据
def getBooksInfo(bookData):
    booksInfo = []
    bookId = bookData['bookId']
    bookName = bookData['bookName']
    chapterTotalCnt = bookData['chapterTotalCnt']
...
# 获取列表数据
def getLinksInfo(bookData, baseUrl):
    links = []
    bookId = bookData['bookId']
    for i in bookData['vs']:
        for j in i['cs']:
            link = baseUrl + '/chapter/' + bookId + '/' + j['id']
            links.append((j['id'], bookId, j['cN'], j['cnt'], j['uuid'], j['fS'], link))
    return links


# print(getLinksInfo(bookData))


def main():
    baseUrl = 'https://www.xs8.cn'
    getBooksInfoData = getBooksInfo(bookData)
    getLinksInfoData = getLinksInfo(bookData, baseUrl)

    getBooksInfoRes = saveBooks(getBooksInfoData)
    if not getBooksInfoRes:
        # for i in getBooksInfoData:
        #     print('书  籍 ： 《%s》 ' % (i[1]))
        print('书籍信息存储失败', getBooksInfoData)

    getLinksInfoRes = saveLinks(getLinksInfoData)
    if not getLinksInfoRes:
        # print('书籍共 %d 章' % len(getLinksInfoData))
