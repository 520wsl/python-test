...
class Spider(Novel):
    ...
    def format_book_catalog_list_data(self, catalog_list, book_id):
        info_list = []
        for i in catalog_list:
            for j in i['cs']:
                src = str(book_id) + '/' + str(j['id'])
                if i['vS'] == 0:
                    src = j['cU']
                info_list.append({
                    'vs': i['vS'],
            ...
            for catalog_info in catalog_info_list:
                ...
                print(
                    '├  书籍 【 %s 】| id 【 %s 】 | book_id 【 %s 】 | chapter_total_cnt 【 %s 】|  catalog_info 【 %s 】  目录存储  ==>  失败' % (
                        item['title'], item['id'], item['book_id'], item['chapter_total_cnt'], str(catalog_info)))
    ...
    def update_book_id_and_catalog(self, info_list):
        ...
        for item in info_list:
            ...
            catalog_id_list = self._mysql_.get_book_catalog_data_list_tup(book_id=item['id'])
            txt_id_list = self._mysql_.get_book_catalog_txt_data_list_tup(book_id=item['id'])
    ...
    def save_book_catalog_txt(self, book_catalog_txt_list):
        ...
        print('├  内容 存储  ==>  失败( %s )' % (len(book_catalog_txt_list)))
    ...
    def spider_book_info_and_catalog_data(self, url, xpath, book_list_html_xpath):
        # 1. 请求页面 获取数据
        html_list = self.get_book_list(url=url, book_list_html_xpath=book_list_html_xpath)
        if len(html_list) <= 0:
            time.sleep(10)
        ...
        # 8. 格式化文章内容
        return self.format_book_catalog_info_list_data(book_info_list=book_info_list_3)


class SpiderModel(Spider):
    # 8、处理模式 1 一条龙（创建小说列表页链接->小说->存储小说->章节->存储章节->内容->存储内容->翻页->小说）
    def run_a_dragon(self, url, xpath, book_list_html_xpath):
        start = time.time()
        book_catalog_info_list = self.spider_book_info_and_catalog_data(url=url, xpath=xpath,
    ...
    def run_save_catalog_list_to_redis(self, url, xpath, book_list_html_xpath):
        start = time.time()
        book_catalog_info_list = self.spider_book_info_and_catalog_data(url=url, xpath=xpath,
                                                                        book_list_html_xpath=book_list_html_xpath)
    ...
    def run_from_redis_get_catalog_save_txt_to_redis(self, catalog_info_list):
        start = time.time()
...
class Run(SpiderModel):
        ...
        while flip_flag:
            ...
            if len(book_page_url_list) > 0:
                ...
                for url in book_page_url_list:
                    self.run_a_dragon(url=url, xpath=xpath, book_list_html_xpath=book_list_html_xpath)
            else:
                if i < maxNum:
    ...
    def save_catalog_list_to_redis(self, xpath, book_list_html_xpath, num=1, maxNum=43200):
        flip_flag = True
        i = 1
        while flip_flag:
            start = time.time()
            ...
            if len(book_page_url_list) > 0:
                ...
                print('├  获取书籍列表链接 %s' % book_page_url_list)
                for url in book_page_url_list:
                    self.run_save_catalog_list_to_redis(url=url, xpath=xpath, book_list_html_xpath=book_list_html_xpath)
            else:
                if i < maxNum:
                    print('├  暂无数据 【 %s 】 60 秒后继续' % i)
                    i += 1
    ...
    def from_redis_get_catalog_save_txt_to_redis(self, num, maxNum):
        ...
        while flip_flag:
            ...
            if len(catalog_info_list) > 0:
                i = 1
                self.run_from_redis_get_catalog_save_txt_to_redis(catalog_info_list=catalog_info_list)
            else:
                if i < maxNum:
                    print('├  暂无数据 【 %s 】 60 秒后继续' % i)
                    i += 1
